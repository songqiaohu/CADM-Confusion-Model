...
'''
source from: Tanha J, Samadi N, Abdi Y, et al. CPSSDS: Conformal prediction for semi-supervised classification on data streams[J]. Information Sciences, 2022, 584: 212-234.
https://github.com/neginsmd/CPSSDS.git
...
drift_point = 100000
dataset = 'doubleline'
filename = './data/' + dataset + '_' + str(drift_point)
dataframe = pd.read_csv('{}.csv'.format(filename), header = None)
dim = dataframe.shape
array = dataframe.values
cosine_all = []
# for drift_point in range(400, 10000, 200):
right = 0
prediction = 0
print(drift_point)
...
chunk_size = 200
Percentage = 0.2  # percentage of labeled data in each chunk 0.05 or 0.1
...
def Prediction_by_CP(num, classifier, X, Y, X_Unlabeled, class_count, sl):
    ...
    labels = np.ones((row, 1), dtype=bool)
...
num_samples = 100000 - X_chunk1.shape[0]
...
while n_samples < num_samples and stream.has_more_samples():
    ...
    if X_Currentchunk.shape[0] >= chunk_size:
        ...
        if mean_kst < 0.05:  # if drift
            ...
            if num == 1:
                ...
                classifier.fit(X_L2, Y_L2) #报错原因是训练数据中只有一个类别
            X_L1 = X_L2.copy()
            Y_L1 = Y_L2.copy()
        else:  # if no drift
            print('information',Informatives)
            [New_X_Labeled, New_Y_Labeled] = Appending_informative_to_nextchunk(X_L2, Y_L2, Informatives,
                                                                                Y_Informatives)
            print('len',len(New_X_Labeled))
            if num == 1:
    ...
    Y_cal1 = Y_cal2.copy()
    X_U1 = X_U2.copy()
    Y_pred = classifier.predict(X_Currentchunk)
    print(Y_pred)
    for k in Y_pred:
        y_pred_all.append(k)
    A.append(accuracy_score(Y_Currentchunk, Y_pred))
